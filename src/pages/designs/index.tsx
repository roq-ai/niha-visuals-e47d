...
import { getDesigns, deleteDesignById } from 'apiSdk/designs';
...
function DesignListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('design', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/designs/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('design', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/designs/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('design', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/designs/edit/${record.id}`} passHref legacyBehavior>
                          <Button
